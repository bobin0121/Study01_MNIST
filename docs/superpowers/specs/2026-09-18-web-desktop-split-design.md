...
이 검사가 전처리를 일부러 비켜가는 것은 순서 때문이다. 순전파는 28x28
입력만 있으면 단독으로 검증할 수 있지만, 전처리는 순전파까지 이어 붙여야
결과를 판단할 수 있다. 따라서 순전파를 먼저 만들어 통과시킨 뒤 전처리를
붙인다.

### 2단계: 전체 파이프라인 정확도

전처리까지 완성되면 `test.html`에 두 번째 구역을 둔다. MNIST
테스트셋 500장을 **전처리 + 순전파 전체**에 통과시켜 전체 정확도를
표시한다. 이때 입력은 원본 28x28을 그대로 쓰지 않고 280x280으로 키워
사용자가 캔버스에 그린 상황에 가깝게 만든다. 그런 다음 경계상자 자르기부터
다시 수행된다.

**축소 방식의 판단 기준**: 면적 평균 축소로 97% 이상이 나오면 그대로
둘 것. 미달하면 그때 Lanczos를 직접 구현해 다시 측정한다. 성능을 미리
가정해서 복잡한 리샘플링을 먼저 쓰지 않는다. 측정 결과는
`web_version/CLAUDE.md`에 기록한다.

`make_test_data.py`는 이 500장도 함께 내보내지만, 로짓은 저장하지 않고
정답 라벨만 담아 파일 크기를 줄인다.
